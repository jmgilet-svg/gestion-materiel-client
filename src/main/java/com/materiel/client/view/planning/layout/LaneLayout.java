...
public final class LaneLayout {
    ...
    public static <T> Map<T, Lane> computeLanes(
            List<T> items,
    ...
    public static <T> Map<T, Lane> computeLanes(List<T> items, StartEnd<T> se, int rowUsableWidth) {
        ...
        for (T it : items) {
            ...
            open.removeIf(o -> !se.end(o).isAfter(s)); // garde overlaps stricts
        ...
        // NB: si tu veux un agrandissement systématique dès 2 overlaps,
        // remplace le calcul ci-dessous par: int tracks = Math.max(1, maxCols);
    ...
    /** Hauteur réelle d'une ligne selon les colonnes nécessaires et la largeur dispo. */
